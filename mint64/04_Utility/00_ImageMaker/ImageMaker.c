...
#include <stdlib.h>
#include <fcntl.h>
#include <unistd.h>     // [변경] io.h 대신 unistd.h 사용 (read, write, close, lseek 등)
#include <sys/types.h>
#include <sys/stat.h>
#include <errno.h>
...
// [변경] 맥/리눅스에는 O_BINARY 플래그가 없으므로 0으로 정의하여 무시함
#ifndef O_BINARY
#define O_BINARY 0
#endif
...
int main(int argc, char *argv[]) {
    ...
    // Disk.img 파일을 생성
    // [변경] S_IREAD | S_IWRITE -> S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH (0644 권한)
    if ((iTargetFd = open("Disk.img", O_RDWR | O_CREAT | O_TRUNC |
                                      O_BINARY, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)) == -1) {
...
int AdjustInSectorSize(int iFd, int iSourceSize) {
    ...
    if (iAdjustSizeToSector != 0) {
        ...
        // [변경] %lu -> %d (iSourceSize는 int형이므로 포맷 경고 수정)
